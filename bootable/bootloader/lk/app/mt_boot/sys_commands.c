...
#ifdef SONY_S1_SUPPORT // SONY uses native Google format for fastboot images
	memmove((void*) hdr.kernel_addr, (ptr + BIMG_HEADER_SZ), hdr.kernel_size);
	memmove((void*) hdr.ramdisk_addr, (ptr + BIMG_HEADER_SZ + kernel_actual),
          hdr.ramdisk_size);
#else
