...
#include <s1boot_helpers.h>
#include <string.h>
#include <s1boot_fastboot.h>
#endif
...
#ifdef SONY_S1_SUPPORT
char g_CMDLINE [1024] = COMMANDLINE_TO_KERNEL;
#else
...
#ifdef SONY_S1_SUPPORT
static s1boot_data_t s1_data = {
  .image_to_boot = BOOT_NORMAL_IMAGE,
  .s1_cmdline_additions = NULL,
  .boot_aid = SONY_S1_BOOT_AID
};
void mt_boot_make_s1_decisions(void)
{
    s1boot_make_s1_decisions(&s1_data);
}
#endif
...
#ifdef SONY_S1_SUPPORT
    // If we got a S1 commandline, append it to the real one
    if (s1_data.s1_cmdline_additions)
    {
      // S1 additions lead with a space
      sprintf(cmdline, "%s%s", cmdline, s1_data.s1_cmdline_additions);
    }
    // Override machine type if S1 lib provided one
    if (s1_data.override_mach != 0)
    {
      machtype = s1_data.override_mach;
    }
#endif // SONY added
    ptr = target_atag_commmandline(ptr, cmdline);
	/*<<EricHsieh,2013/7/8,Add the for SIM detec*/
	ptr = target_atag_sim(ptr);
	/*>>EricHsieh,2013/7/8,Add the for SIM detec*/
...
#ifdef SONY_S1_SUPPORT
  {
    // If a MSN is available, use it for serial number
    u8* buf = NULL;
    u32 sn_size = 0;
    if (s1_ta_simple_read(TA_MSN_UNIT, &buf, &sn_size))
    {
      memset(sn_buf, 0, SN_BUF_LEN);
      memcpy(sn_buf, buf, sn_size > 12 ? 12 : sn_size);
      free(buf);
    }
  }
#endif
