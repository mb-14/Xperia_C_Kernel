...
BOOL write_to_nand(u8* data, u32 length, u32 img_total_len)
{
	...
	if (0 != nand_write_img_ex((u32)(sto_info.image_base_addr+sto_info.bulk_image_offset), (void*)data, length,img_total_len?(img_total_len):(u32)(sto_info.to_write_data_len), &w_length, (u32)(sto_info.image_base_addr),(u32)partition_size, partition_type))
	{
...
int get_mountpoint_from_image(void *data, unsigned sz, char *mountpoint)
{
	...
	if(sz < CHECK_SIZE){
		printf("[get_mountpoint_from_image]sz must be more than one block(%dB)\n",CHECK_SIZE);
		return -1;
	}
	sparse_header = (sparse_header_t *) data;
	...
	if (sparse_header->magic == SPARSE_HEADER_MAGIC)
	{
			...
			if(chunk_header->chunk_type == CHUNK_TYPE_RAW || chunk_data_sz >= 4*1024){
				...
				return __check_mountpoint(data,CHECK_SIZE,mountpoint);
			}else{
...
#ifndef SONY_S1_SUPPORT // SONY uses a different security mechanism for fastboot
