...
//>2013/04/12-23797-stevenchen
		...
		{
			//DSI_clk_HS_mode(1);
			//<2013/06/26-26315-stevenchen, [5860][Coverity 89700] Unchecked call to function.
			DSI_CHECK_RET(DSI_WaitForNotBusy());
			//>2013/06/26-26315-stevenchen
			DSI_SetMode(lcm_params->dsi.mode);
...
DISP_STATUS DISP_Capture_Framebuffer( unsigned int pvbuf, unsigned int bpp, unsigned int is_early_suspended )
{
    ...
    //<2013/07/05-26623-HenryPeng-MTK[ALPS00809651]patch for MonkeyRunner snapshot issue, begin
    disp_path_clear_mem_out_done ();
    //>2013/07/05-26623-HenryPeng-MTK[ALPS00809651]patch for MonkeyRunner snapshot issue, end
    if (is_early_suspended)
    {
        disp_path_get_mutex();
