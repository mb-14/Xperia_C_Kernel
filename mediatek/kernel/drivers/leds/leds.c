...
//<2013/8/20-28344-jessicatseng, [5860] Fix LED issue
 {
  if((g_leds_data[MT65XX_LED_TYPE_BLUE]->delay_on != led->blink_on_time)
  	||(g_leds_data[MT65XX_LED_TYPE_BLUE]->delay_off != led->blink_off_time))
  {
   enablevalue = enablevalue & 0xFB;
   patternvalue = patternvalue & 0xFC;
  }
 }
 
 {
  if((g_leds_data[MT65XX_LED_TYPE_GREEN]->delay_on != led->blink_on_time)
  	||(g_leds_data[MT65XX_LED_TYPE_GREEN]->delay_off != led->blink_off_time))
  {
   enablevalue = enablevalue & 0xF7;
   patternvalue = patternvalue & 0xF3;
  }
 }
 
 {
  if((g_leds_data[MT65XX_LED_TYPE_RED]->delay_on != led->blink_on_time)
  	||(g_leds_data[MT65XX_LED_TYPE_RED]->delay_off != led->blink_off_time))
  {
   enablevalue = enablevalue & 0xEF;
   patternvalue = patternvalue & 0xCF;
  }
 }
//>2013/8/20-28344-jessicatseng
...
//<2013/7/10-26764-jessicatseng, [5860] Add lm3533 debugging code by adb command
...
//<2013/7/10-26764-jessicatseng, [5860] Add lm3533 debugging code by adb command
            rc = device_create_file(g_leds_data[i]->cdev.dev, &dev_attr_lm3533_reg_info);
            if(rc)
            {
