...
const CALIBRATION_LAYOUT_STRUCT CalLayoutTbl[MAX_CALIBRATION_LAYOUT_NUM]=
{
  {//CALIBRATION_LAYOUT_SLIM_LSC1 without Defect //data sheet of excel : "Slim"
    0x00000001, 0x00000000, CAM_CAL_SINGLE_EEPROM_DATA,
    {
      {0x00000001, 0x00000001, 0x00000001, DoCamCalModuleVersion}, //CAMERA_CAM_CAL_DATA_MODULE_VERSION
      {0x00000001, 0x00000000, 0x00000000, DoCamCalPartNumber}, //CAMERA_CAM_CAL_DATA_PART_NUMBER
      {0x00000001, 0x000000fc, 0x000001c4, DoCamCalSingleLsc}, //CAMERA_CAM_CAL_DATA_SHADING_TABLE
      {0x00000001, 0x00000004, 0x00000008, DoCamCalAWBGain}, //CAMERA_CAM_CAL_DATA_3A_GAIN
      {0x00000001, 0x00000000, 0x00000000, DoCamCal3DGeo}  //CAMERA_CAM_CAL_DATA_3D_GEO
    }
  },
  ...
  {//CALIBRATION_LAYOUT_SENSOR_OTP
    ...
    {		
      ...
      {0x00000001, 0x0000001A, 0x00000284, DoCamCalSingleLsc}, //CAMERA_CAM_CAL_DATA_SHADING_TABLE
...
UINT32 DoCamCal2AGain(INT32 CamcamFID, UINT32 start_addr, UINT32 BlockSize, UINT32* pGetSensorCalData)
{
  ...
  UINT8 tempMax = 0;
//<2013/06/26-26325-alberthsiao, [Coverity] 90202~90205 fix Uninitialized scalar variable
  UINT8 tempGain, CalR=0xff, CalGr=0xff, CalGb=0xff, CalB=0xff, FacR=0xff, FacGr=0xff, FacGb=0xff, FacB=0xff;
//>2013/06/26-26325-alberthsiao
  //Structure
  /*
     Byte[0]:Version
    ...
    else
    {
      ...
      if(0x1&AWBAFConfig){
        ...
        {                
          ...
          if ((CalR > CalGr) || (CalB > CalGr)){
              CAM_CAL_ERR("AWB Gain might be wrong, G should be the largest one (CalR= %d, CalGr= %d, CalGb= %d, CalB= %d\n", CalR, CalGr, CalGb, CalB);
          }
          err = CAM_CAL_ERR_NO_ERR;
        }
        else        
        ...
        {                
          ...
          //    tempMax = FacGb;
          //}
          if ((FacR > FacGr) || (FacB > FacGr)){
              CAM_CAL_ERR("AWB Gain might be wrong, G should be the largest one (FacR= %d, FacGr= %d, FacGb= %d, FacB= %d\n", FacR, FacGr, FacGb, FacB);
          }
        ...
        pCamCalData->Single2A.S2aAwb.rUnitGainu4R = (u32)(CalGr*512/CalR);
        pCamCalData->Single2A.S2aAwb.rUnitGainu4G = (u32)512;
        pCamCalData->Single2A.S2aAwb.rUnitGainu4B = (u32)(CalGr*512/CalB);

        pCamCalData->Single2A.S2aAwb.rGoldGainu4R = (u32)(FacGr*512/FacR);
        pCamCalData->Single2A.S2aAwb.rGoldGainu4G = (u32)512;
        pCamCalData->Single2A.S2aAwb.rGoldGainu4B = (u32)(FacGr*512/FacB);
...
//<2013/08/30-28691-alberthsiao,Update camera AF parameters for IMX111
        if (pCamCalData->Single2A.S2aBitEn & 0x1)   {
            if(AFInf >= 58)//difference for facing downward 
                AFInf -=58;
            else
                AFInf=0;
	        if (pCamCalData->Single2A.S2aBitEn & 0x2)  
                AFMacro+=58;////difference for facing upward 
        }
//>2013/08/30-28691-alberthsiao
...
UINT32 DoCamCalSingleLsc(INT32 CamcamFID, UINT32 start_addr, UINT32 BlockSize, UINT32* pGetSensorCalData)
{
  ...
  else
  {
    ...
    else
    {
      ...
      CAM_CAL_LOG_IF(dumpEnable,"====================== dump info. ==================\n");
      CAM_CAL_LOG_IF(dumpEnable,"offset=0x%x, size=0x%x\n", start_addr, BlockSize);
      cam_calCfg.u4Offset = (start_addr&0xFFFF);
      cam_calCfg.u4Length = BlockSize; //sizeof(ucModuleNumber)
      ...
      CAM_CAL_LSC_DATA tmp_LSC;
        {
          int i = 0;
          for (i = 0; i < 644; i++) {
            CAM_CAL_LOG_IF(dumpEnable,"LSC_data[%d] = 0x%2x \n", 
                i,
                pCamCalData->SingleLsc.LscTable.Data[i]); 
          }
        }
      if(pCamCalData->SingleLsc.LscTable.Data[0]==0xff &&
         pCamCalData->SingleLsc.LscTable.Data[1]==0x00 &&
         pCamCalData->SingleLsc.LscTable.Data[2]==0x02 &&
         pCamCalData->SingleLsc.LscTable.Data[3]==0x01)
      {
#if 0
// Backward reading
        tmp_LSC.MtkLcsData.CapTable[380] = pCamCalData->SingleLsc.LscTable.Data[3];
        tmp_LSC.MtkLcsData.CapTable[381] = pCamCalData->SingleLsc.LscTable.Data[2];
        tmp_LSC.MtkLcsData.CapTable[382] = pCamCalData->SingleLsc.LscTable.Data[1];
        tmp_LSC.MtkLcsData.CapTable[383] = pCamCalData->SingleLsc.LscTable.Data[0];
        tmp_LSC.MtkLcsData.MtkLscType = 0x02;
        tmp_LSC.MtkLcsData.PixId = 0x0;
        tmp_LSC.MtkLcsData.TableSize = 384+60;
        tmp_LSC.MtkLcsData.PreviewWH = (pCamCalData->SingleLsc.LscTable.Data[BlockSize-6]<<24)+(pCamCalData->SingleLsc.LscTable.Data[BlockSize-5]<<16)+(pCamCalData->SingleLsc.LscTable.Data[BlockSize-8]<<8)+pCamCalData->SingleLsc.LscTable.Data[BlockSize-7];
        tmp_LSC.MtkLcsData.PreviewOffSet = (pCamCalData->SingleLsc.LscTable.Data[BlockSize-10]<<24)+(pCamCalData->SingleLsc.LscTable.Data[BlockSize-9]<<16)+(pCamCalData->SingleLsc.LscTable.Data[BlockSize-12]<<8)+pCamCalData->SingleLsc.LscTable.Data[BlockSize-11];
        tmp_LSC.MtkLcsData.CaptureWH = (pCamCalData->SingleLsc.LscTable.Data[BlockSize-14]<<24)+(pCamCalData->SingleLsc.LscTable.Data[BlockSize-13]<<16)+(pCamCalData->SingleLsc.LscTable.Data[BlockSize-16]<<8)+pCamCalData->SingleLsc.LscTable.Data[BlockSize-15];
        tmp_LSC.MtkLcsData.CaptureOffSet = (pCamCalData->SingleLsc.LscTable.Data[BlockSize-18]<<24)+(pCamCalData->SingleLsc.LscTable.Data[BlockSize-17]<<16)+(pCamCalData->SingleLsc.LscTable.Data[BlockSize-20]<<8)+pCamCalData->SingleLsc.LscTable.Data[BlockSize-19];
        tmp_LSC.MtkLcsData.PreviewTblSize = (pCamCalData->SingleLsc.LscTable.Data[BlockSize-22]<<24)+(pCamCalData->SingleLsc.LscTable.Data[BlockSize-21]<<16)+(pCamCalData->SingleLsc.LscTable.Data[BlockSize-24]<<8)+pCamCalData->SingleLsc.LscTable.Data[BlockSize-23];
        tmp_LSC.MtkLcsData.CaptureTblSize = (pCamCalData->SingleLsc.LscTable.Data[BlockSize-26]<<24)+(pCamCalData->SingleLsc.LscTable.Data[BlockSize-25]<<16)+(pCamCalData->SingleLsc.LscTable.Data[BlockSize-28]<<8)+pCamCalData->SingleLsc.LscTable.Data[BlockSize-27];

        for(Count=0; Count<5; Count++)
          tmp_LSC.MtkLcsData.PvIspReg[Count] = (pCamCalData->SingleLsc.LscTable.Data[BlockSize-32-4*Count]<<24)+(pCamCalData->SingleLsc.LscTable.Data[BlockSize-31-4*Count]<<16)+(pCamCalData->SingleLsc.LscTable.Data[BlockSize-30-4*Count]<<8)+pCamCalData->SingleLsc.LscTable.Data[BlockSize-29-4*Count];

        for(Count=0; Count<5; Count++)
          tmp_LSC.MtkLcsData.CapIspReg[Count] = (pCamCalData->SingleLsc.LscTable.Data[BlockSize-52-4*Count]<<24)+(pCamCalData->SingleLsc.LscTable.Data[BlockSize-51-4*Count]<<16)+(pCamCalData->SingleLsc.LscTable.Data[BlockSize-50-4*Count]<<8)+pCamCalData->SingleLsc.LscTable.Data[BlockSize-49-4*Count];

        for(Count=0; Count<95; Count++)
          for(Count1=0; Count1<4; Count1++) {
              tmp_LSC.MtkLcsData.CapTable[(Count)*4+Count1] =
                      pCamCalData->SingleLsc.LscTable.Data[BlockSize-69-Count*4-Count1];
              CAM_CAL_LOG_IF(1,"Gain table idx %d\n", (Count)*4+Count1);
          }
        memcpy((void*)&pCamCalData->SingleLsc.LscTable.MtkLcsData, (void*)&tmp_LSC.MtkLcsData,
                sizeof(tmp_LSC.MtkLcsData));
#else
// Forward reading
//        tmp_LSC.MtkLcsData.CapTable[380] = pCamCalData->SingleLsc.LscTable.Data[3];
//        tmp_LSC.MtkLcsData.CapTable[381] = pCamCalData->SingleLsc.LscTable.Data[2];
...
//        tmp_LSC.MtkLcsData.CapTable[383] = pCamCalData->SingleLsc.LscTable.Data[0];
        tmp_LSC.MtkLcsData.MtkLscType = 0x02;
        tmp_LSC.MtkLcsData.PixId = 0x8;
        tmp_LSC.MtkLcsData.TableSize = 144; //lsc_mgr wiil ignore this field
        tmp_LSC.MtkLcsData.PreviewWH = (pCamCalData->SingleLsc.LscTable.Data[-1+6]<<24)+(pCamCalData->SingleLsc.LscTable.Data[-1+5]<<16)+(pCamCalData->SingleLsc.LscTable.Data[-1+8]<<8)+pCamCalData->SingleLsc.LscTable.Data[-1+7];
        tmp_LSC.MtkLcsData.PreviewOffSet = (pCamCalData->SingleLsc.LscTable.Data[-1+10]<<24)+(pCamCalData->SingleLsc.LscTable.Data[-1+9]<<16)+(pCamCalData->SingleLsc.LscTable.Data[-1+12]<<8)+pCamCalData->SingleLsc.LscTable.Data[-1+11];
        tmp_LSC.MtkLcsData.CaptureWH = (pCamCalData->SingleLsc.LscTable.Data[-1+14]<<24)+(pCamCalData->SingleLsc.LscTable.Data[-1+13]<<16)+(pCamCalData->SingleLsc.LscTable.Data[-1+16]<<8)+pCamCalData->SingleLsc.LscTable.Data[-1+15];
        tmp_LSC.MtkLcsData.CaptureOffSet = (pCamCalData->SingleLsc.LscTable.Data[-1+18]<<24)+(pCamCalData->SingleLsc.LscTable.Data[-1+17]<<16)+(pCamCalData->SingleLsc.LscTable.Data[-1+20]<<8)+pCamCalData->SingleLsc.LscTable.Data[-1+19];
        tmp_LSC.MtkLcsData.PreviewTblSize = (pCamCalData->SingleLsc.LscTable.Data[-1+22]<<24)+(pCamCalData->SingleLsc.LscTable.Data[-1+21]<<16)+(pCamCalData->SingleLsc.LscTable.Data[-1+24]<<8)+pCamCalData->SingleLsc.LscTable.Data[-1+23];
//        tmp_LSC.MtkLcsData.CaptureTblSize = (pCamCalData->SingleLsc.LscTable.Data[-1+26]<<24)+(pCamCalData->SingleLsc.LscTable.Data[-1+25]<<16)+(pCamCalData->SingleLsc.LscTable.Data[-1+28]<<8)+pCamCalData->SingleLsc.LscTable.Data[-1+27];
        tmp_LSC.MtkLcsData.CaptureTblSize = (pCamCalData->SingleLsc.LscTable.Data[27]<<24)+(pCamCalData->SingleLsc.LscTable.Data[26]<<16)+(pCamCalData->SingleLsc.LscTable.Data[25]<<8)+pCamCalData->SingleLsc.LscTable.Data[24];

        for(Count=0; Count<5; Count++)
          tmp_LSC.MtkLcsData.PvIspReg[Count] = (pCamCalData->SingleLsc.LscTable.Data[-1+32+4*Count]<<24)+(pCamCalData->SingleLsc.LscTable.Data[-1+31+4*Count]<<16)+(pCamCalData->SingleLsc.LscTable.Data[-1+30+4*Count]<<8)+pCamCalData->SingleLsc.LscTable.Data[-1+29+4*Count];

        for(Count=0; Count<5; Count++)
          tmp_LSC.MtkLcsData.CapIspReg[Count] = (pCamCalData->SingleLsc.LscTable.Data[-1+52+4*Count]<<24)+(pCamCalData->SingleLsc.LscTable.Data[-1+51+4*Count]<<16)+(pCamCalData->SingleLsc.LscTable.Data[-1+50+4*Count]<<8)+pCamCalData->SingleLsc.LscTable.Data[-1+49+4*Count];

        for(Count=0; Count<144; Count++)
          for(Count1=0; Count1<4; Count1++) {
              tmp_LSC.MtkLcsData.CapTable[(Count)*4+Count1] =
                      pCamCalData->SingleLsc.LscTable.Data[-1+69+Count*4+Count1];
              CAM_CAL_LOG_IF(1,"Gain table idx %d\n", (Count)*4+Count1);
          }
        memcpy((void*)&pCamCalData->SingleLsc.LscTable.MtkLcsData, (void*)&tmp_LSC.MtkLcsData,
                sizeof(tmp_LSC.MtkLcsData));
...
#if 1
        {
          ...
          for (i = 0; i < 384; i++) {
            CAM_CAL_LOG_IF(dumpEnable,"CapTable[%d] = 0x%2x \n", 
                i,
                pCamCalData->SingleLsc.LscTable.MtkLcsData.CapTable[i]); 
          }
        }
#else
