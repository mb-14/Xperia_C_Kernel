...
//< 2013/3/20-23122-liulanqing- <[5860][drv] msensor add auto detect function >
#if 0
static int mmc_probe(struct platform_device *pdev);
static int mmc_remove(struct platform_device *pdev);
#endif
extern struct mag_hw* mmc3416x_get_cust_mag_hw(void);
//>2013/3/20-23122-liulanqing
...
//>2013/3/20-23122-liulanqing 
//< 2013/3/20-23122-liulanqing- <[5860][drv] msensor add auto detect function >
static int mmc_probe(void);
static int mmc_remove(void);
static int msensor_init_flag =0;

static struct sensor_init_info mmc3416x_init_info = {
		.name = "mmc3416x",
		.init = mmc_probe,
		.uninit = mmc_remove,
...
//<2013/6/28-26416-davidda, [Pelican][Coverity]89856 89802 89825 89826
//	struct mmc3416x_i2c_data *obj = i2c_get_clientdata(client);
//>2013/6/28-26416-davidda
...
static int mmc3416x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	//< 2013/3/20-23122-liulanqing- <[5860][drv] msensor add auto detect function >
	data->hw =  mmc3416x_get_cust_mag_hw();	
	//> 2013/3/20-23122-liulanqing
...
{
	//< 2013/3/20-23122-liulanqing- <[5860][drv] msensor add auto detect function >
	struct mag_hw *hw =  mmc3416x_get_cust_mag_hw();
	//> 2013/3/20-23122-liulanqing
...
static int __init mmc3416x_init(void)
{
	...
	hwmsen_msensor_add(&mmc3416x_init_info);
	//> 2013/3/20-23122-liulanqing
	return 0;    
}
/*----------------------------------------------------------------------------*/
