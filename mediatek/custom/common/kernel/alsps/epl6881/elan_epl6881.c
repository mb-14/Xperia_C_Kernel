...
int elan_calibaration_read(void)
{
    ...
    struct file *fp_h;
    struct file *fp_l;
//<2013/07/23-27092-kevincheng,Fix P sensor calibration data read back issue
//<2013/07/23-27107-kevincheng,Give the initial value for cal_l , cal_h
    unsigned char cal_h[10] = {0};
    unsigned char cal_l[10] = {0};
//>2013/07/23-27107-kevincheng
//>2013/07/23-27092-kevincheng
    mm_segment_t fs;
    loff_t pos;
    cal_h_val = 0;
    cal_l_val = 0;
...
//<2013/10/17-30215-hankschen,P-sensor formula modified
                     //epld->epl_intthreshold_lvalue = (epld->epl_intthreshold_hvalue)*8/10;
			epld->epl_intthreshold_lvalue = ADC_avg + (threshold_base*7/10);
			printk("[ELAN]hvalue : %d\n",epld->epl_intthreshold_hvalue);
			printk("[ELAN]lvalue : %d\n",epld->epl_intthreshold_lvalue);
//>2013/10/17-30215-hankschen
		}
		Dyna_en = 1;
		ADC_err = 0;
	}
...
static void epl_sensor_irq_do_work(struct work_struct *work)
{
    ...
    if(mode==0x10)
    {
//<2013/06/26-26357-kevincheng,[Coverity]89710
        if(epld->enable_lflag && !epld->ps_irq_enabled)
//>2013/06/26-26387-kevincheng
        {
            elan_epl6881_I2C_Write(client,REG_16,R_TWO_BYTE,0x01,0x00);
            elan_epl6881_I2C_Read(client);
...
//<2013/04/19-24085-kevincheng,Cover Light Sensor can't turn off issue
//<2013/07/09-26699-kevincheng,Add ALSPS prompting
                     sensor_data = (hwm_sensor_data *)buff_out;
//>2013/07/09-26699-kevincheng
