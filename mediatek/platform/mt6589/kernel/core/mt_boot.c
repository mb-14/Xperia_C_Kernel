...
/*<<EricHsieh,2013/7/8,Add the for SIM detec*/
SIMMODE_TYPE g_sim_mode = SINGLE_SIM;
/*>>EricHsieh,2013/7/8,Add the for SIM detec*/
...
static ssize_t boot_show(struct kobject *kobj, struct attribute *a, char *buf)
{
	...
	/*<<EricHsieh,2013/7/8,Add the for SIM detec*/
	else if(!strncmp(a->name, SIMMODE_SYSFS_ATTR, strlen(SIMMODE_SYSFS_ATTR)))
	{
		sprintf(buf,"%d\n",g_sim_mode);
	}
	/*>>EricHsieh,2013/7/8,Add the for SIM detec*/
...
/*<<EricHsieh,2013/7/8,Add the for SIM detect*/
struct attribute simmode_attr = {SIMMODE_SYSFS_ATTR, 0644};
/*>>EricHsieh,2013/7/8,Add the for SIM detect*/
/*<<EricHsieh,2013/8/2,Add the for Band detect*/
struct attribute gpio140oh_attr = {GPIO140_OH, 0644};
struct attribute gpio140ol_attr = {GPIO140_OL, 0644};
struct attribute gpio140in_attr = {GPIO140_IN, 0644};
/*>>EricHsieh,2013/8/2,Add the for Band detect*/
...
/*<<EricHsieh,2013/7/8,Add the for SIM detec*/
static int sim_mode_proc(char *page, char **start, off_t off,int count, int *eof, void *data)
{
    char *p = page;
    int len = 0; 
	//<<JB
	printk("[sim_mode_proc]\n");
	//>>JB
    switch(g_sim_mode)
    {
        case ZERO_SIM :
            p += sprintf(p, "zero_sim\n");
            break;
        case SINGLE_SIM :
            p += sprintf(p, "single_sim\n");
            break;
        case DUAL_SIM :
            p += sprintf(p, "dual_sim\n");
            break;   
        case TRIPLE_SIM :
            p += sprintf(p, "triple_sim\n");
            break;
        case QUAL_SIM :
            p += sprintf(p, "qual_sim\n");
            break;
        default :
            p += sprintf(p, "unknown_sim\n");
            break;
    }  
    *start = page + off;
    len = p - page;
...
/*<<EricHsieh,2013/8/2,Add the for Band detect*/
static int gpio140oh_mode_proc(char *page, char **start, off_t off,int count, int *eof, void *data)
{
    char *p = page;
    int len = 0; 
	//<<JB
	printk("[gpio140oh_mode_proc]\n");
	//>>JB
	mt_set_gpio_mode(GPIO140,GPIO_MODE_GPIO);//Set as GPIO 
	mt_set_gpio_dir(GPIO140,GPIO_DIR_OUT);
	mt_set_gpio_out(GPIO140, GPIO_OUT_ONE);
	p += sprintf(p, "Set GPIO140 High\n");
...
static int gpio140ol_mode_proc(char *page, char **start, off_t off,int count, int *eof, void *data)
{
    char *p = page;
    int len = 0; 
	//<<JB
...
static int gpio140in_mode_proc(char *page, char **start, off_t off,int count, int *eof, void *data)
{
    char *p = page;
    int len = 0; 
	
	//<<JB
	printk("[gpio140in_mode_proc]\n");

	mt_set_gpio_mode(GPIO140,GPIO_MODE_GPIO);//Set as GPIO 
	mt_set_gpio_dir(GPIO140,GPIO_DIR_IN);
	mt_set_gpio_pull_enable(GPIO140,GPIO_PULL_DISABLE);//Enable inpull 
	
	//<<EricHsieh,2013/8/8, Add the TA band mode syc.
	if(mt_get_gpio_in(GPIO140)){
		p += sprintf(p, "VA\n");
		}
	else{
		p += sprintf(p, "ROW\n");
...
static int __init boot_mod_init(void)
{
	...
	/*<<EricHsieh,2013/7/8,Add the for SIM detec*/
	/* create proc entry at /proc/sim_mode */
    create_proc_read_entry("sim_mode", S_IRUGO, NULL, sim_mode_proc, NULL);
	/*>>EricHsieh,2013/7/8,Add the for SIM detec*/
	/*<<EricHsieh,2013/8/2,Add the for Band detect*/
    create_proc_read_entry("gpio140_oh", S_IRUGO, NULL, gpio140oh_mode_proc, NULL);	
    create_proc_read_entry("gpio140_ol", S_IRUGO, NULL, gpio140ol_mode_proc, NULL);	
    create_proc_read_entry("gpio140_in", S_IRUGO, NULL, gpio140in_mode_proc, NULL);
	/*>>EricHsieh,2013/8/2,Add the for Band detect*/
