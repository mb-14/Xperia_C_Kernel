...
//<2013/3/4-22461-jessicatseng, [Pelican] Add charger OVP function using EINT10
#include <cust_eint.h>
#include <mach/eint.h>
//>2013/3/4-22461-jessicatseng

//<2013/3/8-22620-jessicatseng, [Pelican] Modify current sensing 56mohm
#define BQ24157_CURRENT_SENSE_R_56

//<2013/6/18-26047-jessicatseng, [5860] Fix high battery's voltage can't be unlocked by USB or AC charger
//<2013/3/13-22755-jessicatseng, [Pelican] Modify charging current to 1275mA
//<2013/2/20-21926-jessicatseng, [Pelican] Modify AC charging current (1250mA)
#define BQ24157_AC_CHARGING_CURRENT_1150
//#define BQ24157_AC_CHARGING_CURRENT_1275
//>2013/2/20-21926-jessicatseng
//>2013/3/13-22755-jessicatseng
//>2013/3/8-22620-jessicatseng
//>2013/6/18-26047-jessicatseng

//<2013/4/1-23427-jessicatseng, [Pelican] Enable Charger OCP and OVP
#if !defined(HW_PDP)
#define CHARGER_OVP_EINT
#endif
//>2013/4/1-23427-jessicatseng

int Enable_BATDRV_LOG = 2;

int g_low_power_ready = 0;

void pchr_turn_off_charging_bq24157(void);
void pchr_turn_on_charging_bq24157(void);
...
///////////////////////////////////////////////////////////////////////////////////////////
int g_battery_thermal_throttling_flag=1;//2;//3; // 0:nothing, 1:enable batTT&chrTimer, 2:disable batTT&chrTimer, 3:enable batTT, disable chrTimer
int battery_cmd_thermal_test_mode=0;
int battery_cmd_thermal_test_mode_value=0;
int g_battery_tt_check_flag=1;//0; // 0:default enable check batteryTT, 1:default disable check batteryTT
...
#if defined(RECOVERY_CHARGING_FUNCTION)        
static kal_bool battery_temp_error_flag = KAL_FALSE;
static kal_bool battery_temp_high_flag = KAL_FALSE;
static kal_uint8 battery_temp_error_count = 0;
#endif
...
//<2013/6/11-25924-jessicatseng, [5860][ATS00160968][So][VBJ]FIFA Stability - device will power off automatically with battery level as 0 after executing for 20 hours
//<2013/4/11-23740-jessicatseng, [Pelican] Add OVP function of charger IC (bq24157)
//<2013/4/1-23427-jessicatseng, [Pelican] Enable Charger OCP and OVPT
//<2013/3/4-22461-jessicatseng, [Pelican] Add charger OVP function using EINT10
#if defined(CHARGER_OVP_EINT)
#define CHARGER_OVP_EINT_LOW  0
#define CHARGER_OVP_EINT_HIGH 1
static int charger_ovp_eint_state = CHARGER_OVP_EINT_LOW;
static kal_bool g_ovp_ic_charger_ovp = KAL_FALSE;
static kal_bool g_ovp_ic_battery_ovp = KAL_FALSE;
static kal_uint8 charger_ic_battery_ovp_error_count = 0;
static struct work_struct ovp_ic_eint_work;
static struct workqueue_struct * ovp_ic_eint_workqueue = NULL;

static kal_bool g_charger_ic_battery_ovp = KAL_FALSE;
static kal_bool g_charger_ic_charger_ovp = KAL_FALSE;
static struct work_struct charger_ic_state_eint_work;
static struct workqueue_struct * charger_ic_state_eint_workqueue = NULL;
#endif
//>2013/3/4-22461-jessicatseng
//>2013/4/1-23427-jessicatseng
//>2013/4/11-23740-jessicatseng
//>2013/6/11-25924-jessicatseng

//<2013/3/28-23316-jessicatseng, [Pelican] Fix showing charging current is too slow in ATS function
static kal_bool g_get_charging_current_again = KAL_FALSE;
int g_Get_I_Charging(void);
//>2013/3/28-23316-jessicatseng
...
static kal_bool g_recheck_charger_type = KAL_FALSE;
#endif
...
//<2013/3/8-22625-jessicatseng, [Pelican] Add some battery's properties	
    POWER_SUPPLY_PROP_ChargerCurrent,
    POWER_SUPPLY_PROP_USB_Voltage,
    POWER_SUPPLY_PROP_USB_Current,
    POWER_SUPPLY_PROP_MaxAllowedBatteryVoltage,
    POWER_SUPPLY_PROP_RequestedChargerCurrent,
    POWER_SUPPLY_PROP_NrOfSecondsOnSafetyTimer,
//>2013/3/8-22625-jessicatseng	
};
...
//<2013/3/8-22625-jessicatseng, [Pelican] Add some battery's properties	
    case POWER_SUPPLY_PROP_ChargerCurrent:
        val->intval = data->BAT_BatteryAverageCurrent;
        break;

    case POWER_SUPPLY_PROP_USB_Voltage:
        val->intval = data->BAT_ChargerVoltage;
        break;
    ...
    case POWER_SUPPLY_PROP_MaxAllowedBatteryVoltage:
        val->intval = 4200;
        break;

    case POWER_SUPPLY_PROP_RequestedChargerCurrent:
#if defined(BQ24157_CURRENT_SENSE_R_56)
    #if defined(BQ24157_AC_CHARGING_CURRENT_1510)
        val->intval = 1510;
    #elif defined(BQ24157_AC_CHARGING_CURRENT_1390)
        val->intval = 1390;
    #elif defined(BQ24157_AC_CHARGING_CURRENT_1275)
        val->intval = 1275;
    #elif defined(BQ24157_AC_CHARGING_CURRENT_1150)
        val->intval = 1150;
    #elif defined(BQ24157_AC_CHARGING_CURRENT_1030)
        val->intval = 1030;
    #elif defined(BQ24157_AC_CHARGING_CURRENT_910)
        val->intval = 910;
    #else
        val->intval = 785;
    #endif
#else    	
    #if defined(BQ24157_AC_CHARGING_CURRENT_1250)
        val->intval = 1250;
    #elif defined(BQ24157_AC_CHARGING_CURRENT_1150)
        val->intval = 1150;
    #elif defined(BQ24157_AC_CHARGING_CURRENT_1050)
        val->intval = 1050;
    #elif defined(BQ24157_AC_CHARGING_CURRENT_950)
        val->intval = 950;
    #elif defined(BQ24157_AC_CHARGING_CURRENT_850)
        val->intval = 850;
    #elif defined(BQ24157_AC_CHARGING_CURRENT_750)
        val->intval = 750;
    #else
        val->intval = 650;
    #endif
    ...
    case POWER_SUPPLY_PROP_NrOfSecondsOnSafetyTimer:
        val->intval = BMT_status.total_charging_time;
        break;
//>2013/3/8-22625-jessicatseng
...
//<2013/3/8-22643-jessicatseng, [Pelican] Modify thermistor table for DP0 HW
...
void select_charging_curret_bcct(void)
{
    ...
    {
        if(g_bcct_value < 100)        pchr_turn_off_charging_bq24157();
        else if(g_bcct_value < 500)   bq24157_config_interface_liao(0x01,0x38);
        else if(g_bcct_value < 800)   bq24157_config_interface_liao(0x01,0x78);
        else if(g_bcct_value == 800)  bq24157_config_interface_liao(0x01,0xB8);
        else                          bq24157_config_interface_liao(0x01,0x78);     
    }
    else if( (BMT_status.charger_type == STANDARD_CHARGER) ||
             (BMT_status.charger_type == CHARGING_HOST) )
    {
        bq24157_config_interface_liao(0x06,0x70); //set ISAFE
        bq24157_config_interface_liao(0x01,0xF8);
        bq24157_config_interface_liao(0x02,0x8E);
        
        //---------------------------------------------------
        //set IOCHARGE
        if(g_bcct_value < 550)        pchr_turn_off_charging_bq24157();
        else if(g_bcct_value < 650)   bq24157_config_interface_liao(0x04,0x09); 
        else if(g_bcct_value < 750)   bq24157_config_interface_liao(0x04,0x19);
        else if(g_bcct_value < 850)   bq24157_config_interface_liao(0x04,0x29);
        else if(g_bcct_value < 950)   bq24157_config_interface_liao(0x04,0x39);
        else if(g_bcct_value < 1050)  bq24157_config_interface_liao(0x04,0x49);
        else if(g_bcct_value < 1150)  bq24157_config_interface_liao(0x04,0x59);
        else if(g_bcct_value < 1250)  bq24157_config_interface_liao(0x04,0x69);
        else if(g_bcct_value == 1250) bq24157_config_interface_liao(0x04,0x79);
        else                          bq24157_config_interface_liao(0x04,0x19);
        //---------------------------------------------------
...
extern kal_uint32 bq24157_read_interface(kal_uint8 RegNum, kal_uint8 *val, kal_uint8 MASK, kal_uint8 SHIFT);
...
int get_bat_charging_current_level(void)
{
    ...
    {
        //Get current level
        bq24157_read_interface(0x1, &ret_val, 0x3, 0x6);
    ...
    {
        //Get current level
        bq24157_read_interface(0x4, &ret_val, 0x7, 0x4);
...
{
    printk("[BATTERY:bq24157] set_bat_charging_current_limit (%d)\r\n", current_limit);
...
void ChargerHwInit_bq24157(void)
{
    if (Enable_BATDRV_LOG == 1) {
        printk("[MT6320 BAT_probe] ChargerHwInit\n" );
    }
    ...
    if(temp_init_flag==0)
    {
        if(g_enable_high_vbat_spec == 1)
        {
            if(g_pmic_cid == 0x1020)
                bq24157_config_interface_liao(0x06,0x70);    
            else
                bq24157_config_interface_liao(0x06,0x77);
        }
        else
            bq24157_config_interface_liao(0x06,0x70);
...
//<2013/4/11-23740-jessicatseng, [Pelican] Add OVP function of charger IC (bq24157)
#if defined(CHARGER_OVP_EINT)
        bq24157_config_interface_liao(0x00,0xC0);//0x80);
#else
        bq24157_config_interface_liao(0x00,0x80);
#endif
//>2013/4/11-23740-jessicatseng

        bq24157_config_interface_liao(0x01,0x48);//0xb9);
        ...
        if(g_enable_high_vbat_spec == 1)
        {
            if(g_pmic_cid == 0x1020)
                ...
                bq24157_config_interface_liao(0x02,0xaa);
        }
        else
            bq24157_config_interface_liao(0x02,0x8e);
...
//<2013/5/24-25273-jessicatseng, [5860] Set cut-off current to 121mA
        if(g_low_power_ready == 1)
            bq24157_config_interface_liao(0x04,0x09);//0x0B);//0x19);
        else
            bq24157_config_interface_liao(0x04,0x09);//0x0B);//0x1B); //194mA
//>2013/5/24/-25273-jessicatseng
...
void bq24157_set_ac_current(void)
{
    kal_uint8 reg_set_value=0;
    
    if (Enable_BATDRV_LOG == 1) {
        printk("[BATTERY:bq24157] bq24157_set_ac_charging_current \r\n");    
    }    
    ...
    bq24157_config_interface_liao(0x04,0x79);
    bq24157_config_interface_liao(0x05,0x04);
    #endif
    ...
    if(g_enable_high_vbat_spec == 1)
    {
        if(g_pmic_cid == 0x1020)
            ...
            bq24157_config_interface_liao(0x06,0x77); //set ISAFE
    }
    else
        bq24157_config_interface_liao(0x06,0x70); //set ISAFE
     ...
     {          
         bq24157_config_interface_liao(0x05,0x24);
         bq24157_config_interface_liao(0x01,0x78); //for low temp      
         bq24157_config_interface_liao(0x02,0x52); //for 3.9v CV threshold
     }
     else 
     {          
         bq24157_config_interface_liao(0x05,0x04); //release limitation of 325mA  FAN_CON5
         //bq24157_set_iocharge(0x01);               //BQ24157 CON4 IOCHARGE          
         
         if(g_low_power_ready == 1)
             bq24157_config_interface_liao(0x04,0x19);
         else
             bq24157_config_interface_liao(0x04,0x1B); //194mA
         
         bq24157_config_interface_liao(0x01,0xB8);
         ...
         if(g_temp_status == TEMP_POS_10_TO_POS_45)
         {
             if(g_enable_high_vbat_spec == 1)
             {
                ...
                bq24157_config_interface_liao(0x02,0x8E); //for 4.2v CV threshold
         }
         else
         {    
             bq24157_config_interface_liao(0x02,0x7A); //for 4.1v CV threshold  mtk71259 20120720
         }
     }
#else    
//<2013/2/20-21926-jessicatseng, [Pelican] Modify AC charging current (1250mA)
    bq24157_config_interface_liao(0x01,0xC8);//0x88);//0xF8);
//>2013/2/20-21926-jessicatseng    
...
// current sense r 68m
        ...
        reg_set_value=0x70;
    #else
        #if defined(BQ24157_AC_CHARGING_CURRENT_1150)
            reg_set_value=0x60;
        #else
            #if defined(BQ24157_AC_CHARGING_CURRENT_1050)
                reg_set_value=0x50;
            #else
                #if defined(BQ24157_AC_CHARGING_CURRENT_950)
                    reg_set_value=0x40;
                #else
                    #if defined(BQ24157_AC_CHARGING_CURRENT_850)
                        reg_set_value=0x30;
                    #else
                        #if defined(BQ24157_AC_CHARGING_CURRENT_750)
                            reg_set_value=0x20; 
                        #else
                            reg_set_value=0x10;                                                
                        #endif
...
//<2013/5/24-25273-jessicatseng, [5860] Set cut-off current to 121mA
    if(g_low_power_ready == 1)
        reg_set_value += 0x09;//0x0B;//0x09;
    else
        reg_set_value += 0x09;//0x0B;
//>2013/5/24-25273-jessicatseng
    bq24157_config_interface_liao(0x04,reg_set_value);
...
void select_charging_curret_bq24157(void)
{
    ...
    else if (g_ftm_battery_flag) 
    {
        ...
        { 
            ...
            bq24157_config_interface_liao(0x04,0x09);//0x0B);//0x2B); 
            bq24157_config_interface_liao(0x05,0x04); 

            if (Enable_BATDRV_LOG == 1) {
                printk("[BATTERY:bq24157] bq24157_config_interface_liao(0x01,0x48) \r\n");    
            }
        }
        else
        {            
            bq24157_set_ac_current();        

            if (Enable_BATDRV_LOG == 1) {
                printk("[BATTERY:bq24157] bq24157_set_ac_current \r\n");    
            }
        }        
    }
    else 
...
//<2013/4/2-23483-jessicatseng, [Pelican] Add charging function, charging current only has 500mA at 45 ~ 55 degree
#if defined(RECOVERY_CHARGING_FUNCTION)        
//<2013/5/24-25271-jessicatseng, [5860] Set charging current to 500mA if phone is in call mode
        if ((battery_temp_high_flag == KAL_TRUE) || ((2 == call_status) && (BMT_status.bat_vol >= 3950)))
//>2013/5/24-25271-jessicatseng
        {
            if(battery_temp_error_flag == KAL_TRUE)
            {
                pchr_turn_off_charging_bq24157();
            }
            else
            {	
                ...
                bq24157_config_interface_liao(0x04,0x09);//0x0B); 
                bq24157_config_interface_liao(0x05,0x04);
           }
        }        
...
//>2013/4/2-23483-jessicatseng
        {
            ...
            if (g_Support_USBIF == 1)
            {
                ...
                {
                    bq24157_config_interface_liao(0x01,0xbc);
                    if (Enable_BATDRV_LOG == 1) {
                        printk("[BATTERY:bq24157] Disable charging\r\n");    
                    }
                }
                else if (g_usb_state == USB_UNCONFIGURED)
                {
                    bq24157_config_interface_liao(0x01,0x38);
                    if (Enable_BATDRV_LOG == 1) {
                        printk("[BATTERY:bq24157] g_usb_state == USB_UNCONFIGURED \r\n");    
                    }
                }
                else if (g_usb_state == USB_CONFIGURED)
                {
                    bq24157_config_interface_liao(0x01,0x78);
                    if (Enable_BATDRV_LOG == 1) {
                        printk("[BATTERY:bq24157] g_usb_state == USB_CONFIGURED \r\n");    
                    }
                }
                else
                {
                    bq24157_config_interface_liao(0x01,0x78);
                    if (Enable_BATDRV_LOG == 1) {
                        printk("[BATTERY:bq24157] bq24157_config_interface_liao(0x01,0x78); 1\r\n");    
                    }
                }
            }
            else
            {    
                ...
                {
                    bq24157_config_interface_liao(0x05,0x04);
                    //bq24157_set_iocharge(0x02);
                    
                    if(g_low_power_ready == 1)
                        bq24157_config_interface_liao(0x04,0x29);
                    else
                        bq24157_config_interface_liao(0x04,0x2B); //194mA
                    ...
                    {    
                        bq24157_config_interface_liao(0x02,0x7A); //for 4.1v CV threshold  mtk71259 20120720 FA
                    }
                }                       
#else            
                bq24157_config_interface_liao(0x01,0x48);//0x78);
                
//<2013/2/19-21877-jessicatseng, [Pelican] Modify charging current setting for USB
                if(g_enable_high_vbat_spec == 1)
                  {
                    if(g_pmic_cid == 0x1020)
                        bq24157_config_interface_liao(0x02,0x8E);
                    ...
                    bq24157_config_interface_liao(0x02,0x8E);
//>2013/2/19-21877-jessicatseng                    
								
//<2013/3/8-22620-jessicatseng, [Pelican] Modify current sensing 56mohm
                bq24157_config_interface_liao(0x04,0x19);//0x1B);//0x2B); 
//>2013/3/8-22620-jessicatseng

//<2013/4/19-24027-jessicatseng, [5860] Reduce USB charging current(394mA) in PCBA SW
#if defined(ARIMA_PCBA_RELEASE)
                bq24157_config_interface_liao(0x05,0x24);
#else
                bq24157_config_interface_liao(0x05,0x04);
#endif
//>2013/4/19-24027-jessicatseng
                
#endif                
                if (Enable_BATDRV_LOG == 1) {
                    printk("[BATTERY:bq24157] bq24157_config_interface_liao(0x01,0x48); 2\r\n");    
                }            
            }
        } 
        ...
        {   
            ...
            else 
            {
                ...
                {    
                    bq24157_config_interface_liao(0x02,0x7A); //for 4.1v CV threshold mtk71259
                }
            }
#else        
            bq24157_config_interface_liao(0x01,0x48);//0x78);
...
#endif            
            if (Enable_BATDRV_LOG == 1) {
                printk("[BATTERY:bq24157] BMT_status.charger_type == NONSTANDARD_CHARGER \r\n");    
            }
        } 
        else if (BMT_status.charger_type == STANDARD_CHARGER) 
        {
...
//>2013/3/8-22620-jessicatseng
            bq24157_config_interface_liao(0x05,0x04); 
            if (Enable_BATDRV_LOG == 1) {
                printk("[BATTERY:bq24157] bq24157_config_interface_liao(0x01,0x48); 3\r\n");    
            }           
        }        
    }
}
...
void pchr_turn_on_charging_bq24157(void)
{
    ...
    {
        printk("[BATTERY:bq24157] Charger Error, turn OFF charging !\r\n");
        pchr_turn_off_charging_bq24157();
    }
    else if( (get_boot_mode()==META_BOOT) || (get_boot_mode()==ADVMETA_BOOT) )
    {   
        printk("[BATTERY:bq24157] In meta or advanced meta mode, disable charging.\r\n");    
        pchr_turn_off_charging_bq24157();
    }
    else
    {
        ChargerHwInit_bq24157();
    
        if (Enable_BATDRV_LOG == 1) {
            printk("[BATTERY:bq24157] pchr_turn_on_charging !\r\n");
        }
        ...
        if (g_bcct_flag == 1)
        {
            ...
            if (Enable_BATDRV_LOG == 1) {
                printk("[BATTERY:bq24157] select_charging_curret_bcct !\n");
            }
        }
        else
        {
            select_charging_curret_bq24157();
            if (Enable_BATDRV_LOG == 1) {
                printk("[BATTERY:bq24157] select_charging_curret_bq24157 !\n");
            }
        }
        ...
        {
            if (Enable_BATDRV_LOG == 1) {
                printk("[BATTERY:bq24157] charger enable !\r\n");
            }
        }
        else
        {
            //pchr_turn_off_charging_bq24157();
            printk("[BATTERY:bq24157] wait gFG_booting_counter_I_FLAG==2 (%d)\r\n", gFG_booting_counter_I_FLAG);
        }    
    }
}

//<2013/2/15-21813-jessicatseng, [Pelican] Fix phone shows battery full if battery's voltage is over 3.9V and charger is plugged
void pchr_turn_off_charging_bq24157 (void)
{
...
//<2013/3/28-23316-jessicatseng, [Pelican] Fix showing charging current is too slow in ATS function
        if(BMT_status.charger_exist == FALSE)
            g_get_charging_current_again = KAL_TRUE;
//>2013/3/28-23316-jessicatseng
...
//<2013/1/30-21231-jessicatseng, [Pelican] Re-charging if battery's temperature recover to 10 ~ 45 degree
#if defined(RECOVERY_CHARGING_FUNCTION)        
        battery_temp_error_flag = KAL_FALSE;
        battery_temp_high_flag = KAL_FALSE;
        battery_temp_error_count = 0;
#endif
//>2013/1/30-21231-jessicatseng    
//>2013/4/2-23483-jessicatseng
//>2013/4/23-24179-jessicatseng
...
//<2013/3/28-23316-jessicatseng, [Pelican] Fix showing charging current is too slow in ATS function
        g_get_charging_current_again = KAL_FALSE;
//>2013/3/28-23316-jessicatseng

//<2013/6/11-25924-jessicatseng, [5860][ATS00160968][So][VBJ]FIFA Stability - device will power off automatically with battery level as 0 after executing for 20 hours
//<2013/4/11-23740-jessicatseng, [Pelican] Add OVP function of charger IC (bq24157)
#if defined(CHARGER_OVP_EINT)
        g_charger_ic_battery_ovp = KAL_FALSE;
        g_charger_ic_charger_ovp = KAL_FALSE;
        charger_ic_battery_ovp_error_count = 0;			
#endif
//>2013/4/11-23740-jessicatseng
//>2013/6/11-25924-jessicatseng
...
//<2013/3/8-22620-jessicatseng, [Pelican] Modify current sensing 56mohm
#if defined(BQ24157_CURRENT_SENSE_R_56)
        ICharging = (ADC_I_SENSE - ADC_BAT_SENSE)*1000/56; //68mohm
#else    	
...
int BAT_CheckBatteryStatus_bq24157(void)
{
    ...
    if (Enable_BATDRV_LOG == 1) {
        printk("[BATTERY:ADC:bq24157] VCHR:%d BAT_SENSE:%d I_SENSE:%d TBAT:%d (%d)\n", 
...
//>2013/4/23-24179-jessicatseng        
    ...
    if (Enable_BATDRV_LOG >= 1) {
        printk("[BATTERY:AVG:bq24157] BatTemp:%d Vbat:%d VBatSen:%d SOC:%d ChrDet:%d Vchrin:%d Icharging:%d ChrType:%d USBstate:%d\r\n", 
    ...
    if (Enable_BATDRV_LOG == 1) {        
        printk("[BATTERY:FG:bq24157] %d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\r\n", 
           BMT_status.temperature ,BMT_status.bat_vol, BMT_status.ADC_BAT_SENSE, BMT_status.SOC, 
...
//<2013/4/23-24179-jessicatseng, [5860] Modify the range of charging temperature
#if defined(RECOVERY_CHARGING_FUNCTION)
        if(battery_temp_error_count < 2)
        {
            battery_temp_error_count ++;
            return PMU_STATUS_OK;
        }
        else
        {
            ...
            BMT_status.bat_charging_state = CHR_ERROR;
            battery_temp_error_flag = KAL_TRUE;
            battery_temp_error_count = 3;
            return PMU_STATUS_FAIL;       
        }
#else
        printk(  "[BATTERY] Battery Under Temperature or NTC fail !!\n\r");                
...
//<2013/4/2-23483-jessicatseng, [Pelican] Add charging function, charging current only has 500mA at 45 ~ 55 degree
#if defined(RECOVERY_CHARGING_FUNCTION)
    if ((BMT_status.temperature >= REDUCE_CHARGE_CURRENT_TEMPERATURE) && 
          (BMT_status.temperature < MAX_CHARGE_TEMPERATURE))
    {
        battery_temp_high_flag = KAL_TRUE;                
    }

    if ((battery_temp_high_flag == KAL_TRUE) && (BMT_status.temperature < REDUCE_CHARGE_CURRENT_TEMPERATURE))
    {
        battery_temp_high_flag = KAL_FALSE;
    }
#endif
...
//<2013/3/4-22461-jessicatseng, [Pelican] Add charger OVP function using EINT10
        ...
        if((BMT_status.charger_vol >= V_CHARGER_MAX) 
        	|| (KAL_TRUE == g_ovp_ic_charger_ovp)
        	|| (KAL_TRUE == g_charger_ic_charger_ovp))
#else
        if ( BMT_status.charger_vol >= V_CHARGER_MAX )
#endif        	
//>2013/3/4-22461-jessicatseng
...
//<2013/6/11-25924-jessicatseng, [5860][ATS00160968][So][VBJ]FIFA Stability - device will power off automatically with battery level as 0 after executing for 20 hours
#if defined(CHARGER_OVP_EINT)
        if(KAL_TRUE == g_ovp_ic_battery_ovp)
        {
            printk("[BATTERY] OVP IC Battery over voltage !!\n");
            BMT_status.bat_charging_state = CHR_ERROR;
            return PMU_STATUS_FAIL;
        }

        if(KAL_TRUE == g_charger_ic_battery_ovp)
        {
            if(charger_ic_battery_ovp_error_count < 5)
            {
                charger_ic_battery_ovp_error_count++;
                g_charger_ic_battery_ovp = KAL_FALSE;
                return PMU_STATUS_OK;		
            }
            else
            {
                printk("[BATTERY] Charger IC Battery over voltage !!\n");
                charger_ic_battery_ovp_error_count = 5;						        
                BMT_status.bat_charging_state = CHR_ERROR;
                return PMU_STATUS_FAIL;
            }
        }
...
#if defined(RECOVERY_CHARGING_FUNCTION)        
		if((battery_temp_error_flag == KAL_TRUE) && 
			(BMT_status.temperature < MAX_RECOVERY_CHARGE_TEMPERATURE) &&
			(BMT_status.temperature >= MIX_RECOVERY_CHARGE_TEMPERATURE))
		{
			BMT_status.bat_charging_state = CHR_PRE;
			battery_temp_error_flag = KAL_FALSE;
			battery_temp_high_flag = KAL_FALSE;
...
//<2013/6/11-25924-jessicatseng, [5860][ATS00160968][So][VBJ]FIFA Stability - device will power off automatically with battery level as 0 after executing for 20 hours
#if defined(CHARGER_OVP_EINT)
		if((BMT_status.bat_vol <= 3800) && (KAL_TRUE == g_charger_ic_battery_ovp)
        && (charger_ic_battery_ovp_error_count >= 5))
		{
			printk("recovery charging\n");
			BMT_status.bat_charging_state = CHR_PRE;
			g_charger_ic_battery_ovp = KAL_FALSE;
			charger_ic_battery_ovp_error_count = 0;
		}
...
#if defined(CHARGER_OVP_EINT)
            xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[BATTERY] g_ovp_ic_charger_ovp=%d, g_charger_ic_charger_ovp=%d\n",
                g_ovp_ic_charger_ovp, g_charger_ic_charger_ovp);
#endif                
        }
        else
        {
...
#else
//Begin_Arima_HCSW7_20130603_SandyTan - add for low temperature
	#if 0
        if( (BMT_status.temperature >= MAX_CHARGE_TEMPERATURE) || 
            (BMT_status.temperature < MIN_CHARGE_TEMPERATURE)
            )
	#endif
//End_Arima_HCSW7_20130603_SandyTan
        if( (BMT_status.temperature >= MAX_CHARGE_TEMPERATURE)
            )
#endif
//>2013/4/23-24179-jessicatseng      
...
//Begin_Arima_HCSW7_20130603_SandyTan - add for low temperature
        if((BMT_status.temperature <= MIN_CHARGE_TEMPERATURE) &&(get_charger_detect_status())) 
        { 
            g_BatteryNotifyCode |= 0x0020; 
            xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[BATTERY] bat_temp(%d) out of range && charger exit\n", BMT_status.temperature);
        } 
        else 
        { 
            g_BatteryNotifyCode &= ~(0x0020); 
        ...
        if (Enable_BATDRV_LOG == 1) { 
            xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[BATTERY] BATTERY_NOTIFY_CASE_0006 (%x)\n", 
                g_BatteryNotifyCode); 
        }
//End_Arima_HCSW7_20130603_SandyTan    
#endif
...
//<2013/2/8-21774-jessicatseng, [Pelican] Add prompt if battery's voltage is over 4.3V
        ...
        if((BMT_status.bat_vol > 4300) 
        	|| ((KAL_TRUE == g_charger_ic_battery_ovp) && (charger_ic_battery_ovp_error_count >= 5))
        	|| (KAL_TRUE == g_ovp_ic_battery_ovp))
...
#endif
        {
            ...
            xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[BATTERY] bat_vlot(%ld) > 4300mV\n", BMT_status.bat_vol);
#if defined(CHARGER_OVP_EINT)
            xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[BATTERY] g_ovp_ic_battery_ovp=%d, g_charger_ic_battery_ovp=%d\n", g_ovp_ic_battery_ovp, g_charger_ic_battery_ovp);
#endif
        }
        else
        {
...
//<2013/3/25-23166-jessicatseng, [Pelican] Modify charging safety timer, AC charger (4 hours), USB (8 hours)
            if((BMT_status.charger_type == STANDARD_CHARGER) || (BMT_status.charger_type == CHARGING_HOST))
            {
	            if(BMT_status.total_charging_time >= MAX_CHARGING_TIME_AC_CHARGER)
...
//<2013/5/31-25553-jessicatseng, [5860] Send a message to notify AP layer battery's temperature is under range
#if defined(BATTERY_NOTIFY_CASE_0006)
#if defined(RECOVERY_CHARGING_FUNCTION)
        if((/*(BMT_status.temperature >= MAX_CHARGE_TEMPERATURE) || */
            (BMT_status.temperature < MIN_CHARGE_TEMPERATURE)) && 
            (battery_temp_error_count >= 3))
...
//<2013/5/10-24794-jessicatseng, [5860] Disable to check whether battery exist or not    
//<2013/4/19-24028-jessicatseng, [5860] Enable to check battery's temperature   
//<2013/1/21-20662-jessicatseng, [Pelican] Don't check if battery existing or not temporarily
    return;
//>2013/1/21-20662-jessicatseng
//>2013/4/19-24028-jessicatseng
//>2013/5/10-24794-jessicatseng
    ...
    if( baton_count >= 3)
    {
        ...
        {
            printk("[BATTERY] Battery is not exist, power off BQ24157 and system (%d)\n", baton_count);
            pchr_turn_off_charging_bq24157();
            arch_reset(0,NULL);      
        }
    }    
...
void BAT_thread_bq24157(void)
{    
    kal_uint32 bq24157_status=0;
    int i=0;
    int BAT_status = 0;
    //kal_uint32 tmp32;
    int ret_val=0;
...
//<2013/3/29-23361-jessicatseng, [Pelican] Add re-check charger type 
#if defined(RECHECK_CHARGER_TYPE)    	
        else if((BMT_status.charger_type == NONSTANDARD_CHARGER) && (g_recheck_charger_type == KAL_FALSE))
        {
            printk("[BATTERY_TOP] recheck charger type start\n");
            g_recheck_charger_type = KAL_TRUE;
            CHR_Type_num = hw_charger_type_detection();
            BMT_status.charger_type = CHR_Type_num;
            ...
            printk("[BATTERY_TOP] recheck charger type end CHR_Type_num=%d\n", CHR_Type_num);
        }
#endif
//>2013/3/29-23361-jessicatseng
    }
...
//<2013/3/29-23361-jessicatseng, [Pelican] Add re-check charger type 
#if defined(RECHECK_CHARGER_TYPE)   
        g_recheck_charger_type = KAL_FALSE;
#endif
//>2013/3/29-23361-jessicatseng
    ...
    BAT_status = BAT_CheckBatteryStatus_bq24157();
    if( BMT_status.bat_charging_state == CHR_ERROR || BAT_status == PMU_STATUS_FAIL)
        g_Battery_Fail = KAL_TRUE;
    else
        g_Battery_Fail = KAL_FALSE;

	//<2013/9/24-maoyichou, For lab test add: Charging stop when battery notify test mode is 5.
	if(g_BN_TestMode == 0x0005)
		g_Battery_Fail = KAL_TRUE;
	//>2013/9/24-maoyichou, For lab test add: Charging stop when battery notify test mode is 5.
...
//<2013/5/24-25284-jessicatseng, [5860]Recover charging as battery's voltage is below 3.8 if charging is stopped by safety timer
...
//<2013/4/24-24262-jessicatseng, [5860] Battery's voltage charge up to 4.0V if temperature is higher than 45 degree
//<2013/4/12-23795-jessicatseng, [Pelican] Charge up to 4.0V in call mode
        printk("[BATTERY:bq24157] call_status = %d\n", call_status);
        if(((2 == call_status) || (KAL_TRUE == battery_temp_high_flag))
        	&&(BMT_status.bat_vol >= 4000)) 
        {
            pchr_turn_off_charging_bq24157();
            return;        	
        }	
...
//<2013/2/15-21813-jessicatseng, [Pelican] Fix phone shows battery full if battery's voltage is over 3.9V and charger is plugged
        ...
        pchr_turn_on_charging_bq24157();        
        if (Enable_BATDRV_LOG >= 1) {
            printk("[BATTERY:bq24157] Total charging timer=%ld \n", 
                BMT_status.total_charging_time);    
        }
//>2013/2/15-21813-jessicatseng        

        bq24157_status = bq24157_get_chip_status();
        
        /* check battery full */
        if( bq24157_status == 0x2 )
        {
            if(BMT_status.bat_vol >= 3900)
            {
                ...
                g_HW_Charging_Done = 1;            
                //pchr_turn_off_charging_bq24157();
                printk("[BATTERY:bq24157] Battery real full and disable charging (%d) \n", bq24157_status); 
                return;
            }
            else
            {
                printk("[BATTERY:bq24157] HW Battery real full ?? (%d,%d) \n", bq24157_status, BMT_status.bat_vol);
                pchr_turn_off_charging_bq24157();
            }
        }

//<2013/3/28-23316-jessicatseng, [Pelican] Fix showing charging current is too slow in ATS function
        if((g_get_charging_current_again == KAL_TRUE) && (gFG_booting_counter_I_FLAG == 2))
        {
            mdelay(15);
            g_get_charging_current_again = KAL_FALSE;
            mt6320_battery_main.BAT_BatteryAverageCurrent = g_Get_I_Charging();
            //printk("g_BatteryAverageCurrent=%d, BAT_BatteryAverageCurrent=%d\n", g_BatteryAverageCurrent, mt6320_battery_main.BAT_BatteryAverageCurrent);
            power_supply_changed(&(mt6320_battery_main.psy));    
        }
//>2013/3/28-23316-jessicatseng
...
///////////////////////////////////////////////////////////////////////////////////////////
int g_FG_init = 0;
extern unsigned int g_bq24157_rdy_flag;
...
//<2013/3/4-22461-jessicatseng, [Pelican] Add charger OVP function using EINT10
#if defined(CHARGER_OVP_EINT)
void ovp_ic_eint_work_callback(struct work_struct *work)
{
    printk("ovp_ic_eint_work_callback start charger_ovp=%d, battery_ovp=%d, eint_state=%d\n", g_ovp_ic_charger_ovp, g_ovp_ic_battery_ovp, charger_ovp_eint_state);
    
    if(charger_ovp_eint_state == CHARGER_OVP_EINT_HIGH)
    {
        charger_ovp_eint_state = CHARGER_OVP_EINT_LOW;
        g_ovp_ic_charger_ovp = KAL_FALSE;
        g_ovp_ic_battery_ovp = KAL_FALSE;
    }
    else
    {		    	
        charger_ovp_eint_state = CHARGER_OVP_EINT_HIGH;
        
        BMT_status.ADC_BAT_SENSE = get_bat_sense_volt(1);
        //BMT_status.ICharging = g_Get_I_Charging();
        
        if(BMT_status.ADC_BAT_SENSE > 4300)
            g_ovp_ic_battery_ovp = KAL_TRUE;
        else
            g_ovp_ic_charger_ovp = KAL_TRUE;
        	    
        printk("BMT_status.ADC_BAT_SENSE = %d\n", BMT_status.ADC_BAT_SENSE);
    }				    
    
    mt65xx_eint_set_polarity(CUST_EINT_CHARGER_OVP_NUM, charger_ovp_eint_state);
    mt65xx_eint_unmask(CUST_EINT_CHARGER_OVP_NUM);
    
    //printk("ovp_ic_eint_work_callback end g_charger_ovp_stop_charging=%d, charger_ovp_eint_state=%d", g_charger_ovp_stop_charging, charger_ovp_eint_state);
}

void charger_ovp_eint_func(void)
{
    printk("charger_ovp_eint_func\n");
    
    queue_work(ovp_ic_eint_workqueue, &ovp_ic_eint_work);	

    return 0;
}

static void charger_ovp_eint_setup(void)
{
    mt_set_gpio_mode(GPIO_CHARGER_OVP_EINT_PIN, GPIO_CHARGER_OVP_EINT_PIN_M_EINT);
    mt_set_gpio_dir(GPIO_CHARGER_OVP_EINT_PIN, GPIO_DIR_IN);
    mt_set_gpio_pull_enable(GPIO_CHARGER_OVP_EINT_PIN, GPIO_PULL_DISABLE); //To disable GPIO PULL.
    mt65xx_eint_set_sens(CUST_EINT_CHARGER_OVP_NUM, CUST_EINT_CHARGER_OVP_SENSITIVE);
    mt65xx_eint_set_hw_debounce(CUST_EINT_CHARGER_OVP_NUM, CUST_EINT_CHARGER_OVP_DEBOUNCE_CN);
    mt65xx_eint_registration(CUST_EINT_CHARGER_OVP_NUM, CUST_EINT_CHARGER_OVP_DEBOUNCE_EN, CUST_EINT_CHARGER_OVP_POLARITY, charger_ovp_eint_func, 0);
...
void charger_ic_state_eint_work_callback(struct work_struct *work)
{
    kal_uint32 bq24157_status = 0, fault_reason = 0;
    
    bq24157_status = bq24157_get_chip_status();
        
    printk("charger_ic_state_eint_work_callback bq24157_status=0x%x\n", bq24157_status);
    
    if(KAL_TRUE == upmu_is_chr_det())
    {	
        if(bq24157_status == 0x03)
        {
            fault_reason = bq24157_get_fault_reason();
                    	
            switch(fault_reason)
            {        	
//<2013/6/11-25924-jessicatseng, [5860][ATS00160968][So][VBJ]FIFA Stability - device will power off automatically with battery level as 0 after executing for 20 hours
                //case 0x01: g_charger_ic_charger_ovp = KAL_TRUE; break;           	
                case 0x04: g_charger_ic_battery_ovp = KAL_TRUE; break;
//>2013/6/11-25924-jessicatseng                	
            }
        }
    }
    ...
    {
        printk("charger_ic_state_eint_work_callback plug out\n");    	
    }	
    	       
    mt65xx_eint_unmask(CUST_EINT_CHARGER_IC_STATUS_NUM);
}

void charger_ic_state_eint_func(void)
{
    printk("charger_ic_state_eint_func\n");
    
    queue_work(charger_ic_state_eint_workqueue, &charger_ic_state_eint_work);	

    return 0;
}

static void charger_ic_eint_setup(void)
{
    mt_set_gpio_mode(GPIO_CHARGER_IC_STATUS_PIN, GPIO_CHARGER_IC_STATUS_PIN_M_EINT);
    mt_set_gpio_dir(GPIO_CHARGER_IC_STATUS_PIN, GPIO_DIR_IN);
    mt_set_gpio_pull_enable(GPIO_CHARGER_IC_STATUS_PIN, GPIO_PULL_DISABLE); //To disable GPIO PULL.
    mt65xx_eint_set_sens(CUST_EINT_CHARGER_IC_STATUS_NUM, CUST_EINT_CHARGER_IC_STATUS_SENSITIVE);
    mt65xx_eint_set_hw_debounce(CUST_EINT_CHARGER_IC_STATUS_NUM, CUST_EINT_CHARGER_IC_STATUS_DEBOUNCE_CN);
    mt65xx_eint_registration(CUST_EINT_CHARGER_IC_STATUS_NUM, CUST_EINT_CHARGER_IC_STATUS_DEBOUNCE_EN, CUST_EINT_CHARGER_IC_STATUS_POLARITY, charger_ic_state_eint_func, 0);
...
//<2013/3/4-22461-jessicatseng, [Pelican] Add charger OVP function using EINT10
#if defined(CHARGER_OVP_EINT)    
    ovp_ic_eint_workqueue = create_singlethread_workqueue("ovp_ic_eint");
    INIT_WORK(&ovp_ic_eint_work, ovp_ic_eint_work_callback);

    charger_ovp_eint_setup();
//>2013/3/4-22461-jessicatseng

    charger_ic_state_eint_workqueue = create_singlethread_workqueue("charger_ic_state_eint");
    INIT_WORK(&charger_ic_state_eint_work, charger_ic_state_eint_work_callback);

    charger_ic_eint_setup();
#endif    
//>2013/4/11-23740-jessicatseng
...
//<2013/4/11-23740-jessicatseng, [Pelican] Add OVP function of charger IC (bq24157)
#if defined(CHARGER_OVP_EINT)    
    destroy_workqueue(charger_ic_state_eint_workqueue);
    destroy_workqueue(ovp_ic_eint_workqueue);
#endif    
//>2013/4/11-23740-jessicatseng
