...
//<2013/09/03-28803-stevenchen, [5860] Create proc/emmc_vendor to distinguish emmc flash.
static struct proc_dir_entry *proc_emmc_vendor;
//>2013/09/03-28803-stevenchen
...
//<2013/09/03-28803-stevenchen, [5860] Create proc/emmc_vendor to distinguish emmc flash.
static unsigned int emmc_manfid=0;
static int emmc_vendor_read_proc(char *page, char **start, off_t off,int count, int *eof, void *data)
{
	char *p = page;
	int len = 0; 
	int lcm_id=0;
...
// End_Arima_HCSW7_20130429_JohnnyZheng - [MTK patch] ALPS00595423
    ...
    if (is_card_sdio(host)) 
    {
//<2013/08/20-28293-kittychou, [5860][ROW] Fix SDIO errors occur can't power on issue.
        if(host->clk_gate_count > 0){
            host->error = 0;
            return -EBUSY;
        }
//>2013/08/20-28293-kittychou        
...
//<2013/09/03-28803-stevenchen, [5860] Create proc/emmc_vendor to distinguish emmc flash.
	if ((proc_emmc_vendor = create_proc_entry( "emmc_vendor", 0, NULL )))
		proc_emmc_vendor->read_proc = emmc_vendor_read_proc;
//>2013/09/03-28803-stevenchen
	#endif /* MTK_EMMC_SUPPORT && CONFIG_PROC_FS */
